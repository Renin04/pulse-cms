...
const BLOCKS = [
  { phase: 'L-4', type: 'quiz', search: 'Quiz', label: 'Quiz' },
  { phase: 'L-4', type: 'poll', search: 'Poll', label: 'Poll' },
  { phase: 'L-4', type: 'survey', search: 'Survey', label: 'Survey' },
  { phase: 'L-4', type: 'flashcard', search: 'Flashcard', label: 'Flashcard' },
  { phase: 'L-4', type: 'accordion', search: 'Accordion', label: 'Accordion' },
  { phase: 'L-4', type: 'tabs', search: 'Tabs', label: 'Tabs' },
  { phase: 'L-4', type: 'toggle', search: 'Toggle', label: 'Toggle' },
  { phase: 'L-4', type: 'spoiler', search: 'Spoiler', label: 'Spoiler' },
];
...
async function run() {
  ...
  await page.screenshot({ path: path.join(SCREENSHOT_DIR, 'L-4-00-initial.png') });
  ...
  for (const block of BLOCKS) {
    ...
    try {
      ...
      await delay(700);
      await page.screenshot({ path: path.join(SCREENSHOT_DIR, `L-4-${block.type}.png`) });
  ...
  await delay(500);
  await page.screenshot({ path: path.join(SCREENSHOT_DIR, 'L-4-zz-final.png') });
