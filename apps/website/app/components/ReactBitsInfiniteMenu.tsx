...
class InfiniteGridMenu {
  ...
  #onControlUpdate(deltaTime) {
    ...
    if (!this.control.isPointerDown) {
      if (this.#snapToItemIndex !== null) {
        const idx = this.#snapToItemIndex % this.instancePositions.length;
        const worldPos = vec3.normalize(vec3.create(), this.#getVertexWorldPosition(idx));
        this.control.snapTargetDirection = worldPos;
        const front = vec3.fromValues(0, 0, -1);
        const alignment = vec3.dot(front, worldPos);
        if (alignment > 0.97) {
          this.onActiveItemChange(this.#snapToItemIndex % Math.max(1, this.items.length));
          this.#snapToItemIndex = null;
        }
      } else {
...
interface ReactBitsInfiniteMenuProps {
  items: MenuItem[];
  scale?: number;
...
export interface InfiniteMenuRef {
  rotateToItem: (index: number) => void;
}

const ReactBitsInfiniteMenu = React.forwardRef<InfiniteMenuRef, ReactBitsInfiniteMenuProps>(
  function ReactBitsInfiniteMenu(
    { items = [], scale = 1.0, onActiveItemChange },
    ref
  ) {
...
export default ReactBitsInfiniteMenu;
